...
/** Authenticated device-only reader for upcoming Calendar projections. */
public final class CalendarEventClient implements AutoCloseable {
    public interface Callback { void onEvents(JSONObject value); void onFailure(); }
    private final ExecutorService worker = Executors.newSingleThreadExecutor();
    private final Handler main = new Handler(Looper.getMainLooper());
    private final AtomicBoolean closed = new AtomicBoolean();

    public void loadUpcoming(Context context, Callback callback) {
        worker.execute(() -> {
            HttpURLConnection connection = null;
            try {
                String token = new RuntimeSecretStore(context.getApplicationContext()).loadLocalApiToken();
                connection = (HttpURLConnection) new URL("http://127.0.0.1:8765/v1/calendar/upcoming").openConnection();
                ...
                try (InputStream input = connection.getInputStream()) {
                    ...
                    if (!closed.get()) main.post(() -> callback.onEvents(value));
                }
