...
public final class CalendarPageView extends View implements AutoCloseable {
    ...
    static String friendly(String value,boolean allDay){if(value==null||value.isBlank())return "";try{OffsetDateTime parsed=OffsetDateTime.parse(value);if(!allDay)parsed=parsed.atZoneSameInstant(ZoneId.systemDefault()).toOffsetDateTime();return parsed.format(DateTimeFormatter.ofPattern("EEE, MMM d · h:mm a"));}catch(Exception ignored){return value;}}
