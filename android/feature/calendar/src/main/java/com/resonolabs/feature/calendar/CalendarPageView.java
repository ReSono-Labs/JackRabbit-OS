...
public final class CalendarPageView extends View implements AutoCloseable {
    ...
    public CalendarPageView(Activity activity,Runnable openVoice){super(activity);this.openVoice=openVoice;setFocusable(true);setContentDescription("Upcoming calendar events");}
    ...
    private void header(Canvas c){ReSonoTheme.text(c,paint,detail?"‹":"",25,53,43,ReSonoTheme.INK,Paint.Align.LEFT,false);ReSonoTheme.text(c,paint,"Calendar",55,45,30,ReSonoTheme.INK,Paint.Align.LEFT,false);ReSonoTheme.text(c,paint,detail?"EVENT DETAILS":"UPCOMING",56,67,16,ReSonoTheme.MUTED,Paint.Align.LEFT,true);paint.setColor(ReSonoTheme.MINT);c.drawCircle(372,40,5,paint);ReSonoTheme.text(c,paint,"LIVE",384,45,14,ReSonoTheme.MINT,Paint.Align.LEFT,true);}
    ...
    private void drawDetail(Canvas c){JSONObject item=current();paint.setColor(ReSonoTheme.PANEL);c.drawRoundRect(new RectF(18,95,462,510),26,26,paint);paint.setStyle(Paint.Style.STROKE);paint.setStrokeWidth(2);paint.setColor(0xffff5ca8);c.drawRoundRect(new RectF(18,95,462,510),26,26,paint);paint.setStyle(Paint.Style.FILL);c.save();c.clipRect(30,106,450,466);float y=142-detailScroll;y=wrapped(c,item.optString("title","Untitled event"),30,y,32,38,420,ReSonoTheme.INK)+18;y=field(c,"STARTS",friendly(item.optString("startsAt")),y);y=field(c,"ENDS",friendly(item.optString("endsAt")),y);y=field(c,"LOCATION",item.optString("location"),y);y=field(c,"CALENDAR",item.optString("calendar"),y);y=field(c,"ORGANIZER",item.optString("organizer"),y);y=field(c,"DESCRIPTION",item.optString("description"),y);maxDetailScroll=Math.max(0,y+detailScroll-450);detailScroll=Math.min(detailScroll,maxDetailScroll);c.restore();paint.setColor(ReSonoTheme.PANEL_RAISED);c.drawRoundRect(new RectF(30,466,450,506),10,10,paint);ReSonoTheme.text(c,paint,"DRAG TO SCROLL",442,494,15,ReSonoTheme.MUTED,Paint.Align.RIGHT,true);boolean editable=item.optBoolean("editable");paint.setColor(editable?ReSonoTheme.MINT:ReSonoTheme.PANEL_RAISED);c.drawRoundRect(new RectF(18,528,462,592),18,18,paint);ReSonoTheme.text(c,paint,editable?"EDIT WITH VOICE":"READ ONLY · CONNECTED CALENDAR",240,568,20,editable?ReSonoTheme.BACKGROUND:ReSonoTheme.MUTED,Paint.Align.CENTER,true);}
