...
def contracts() -> tuple[TaskToolContract, ...]:
    ...
    return (
        ...
        TaskToolContract("tasks_add", "Prepare a plain-text task. Preserve any date or future wording the user supplied (for example, 'tomorrow') in the task text, but tell the user Tasks cannot schedule it or send a reminder. Read the exact prepared text back and ask before confirming.", "local_write", _schema({"text": {"type": "string"}}, ("text",))),
        TaskToolContract("tasks_edit", "Prepare replacement plain-text task text. Preserve any date or future wording the user supplied in the text, but tell the user Tasks cannot schedule it or send a reminder. Read the exact replacement text back and ask before confirming.", "local_write", _schema({**task_id, "text": {"type": "string"}}, ("taskId", "text"))),
