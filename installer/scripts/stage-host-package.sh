...
output_root="$installer_root/dist/packages/jackrabbit-${platform}-current-v0.1"
mkdir -p "$cache_root" "$installer_root/dist/packages"
[[ ! -e "$output_root" ]] || { printf 'JR-HOST-PACKAGE-EXISTS: remove the existing package first: %s\n' "$output_root" >&2; exit 1; }
mkdir -p "$output_root/bin" "$output_root/release/tools" "$output_root/drivers"
...
case "$platform" in
  windows-x64)
    ...
    cp "$temporary/platform-tools/platform-tools/fastboot.exe" "$output_root/release/tools/fastboot.exe"
    cp "$temporary/platform-tools/platform-tools/AdbWinApi.dll" "$temporary/platform-tools/platform-tools/AdbWinUsbApi.dll" "$temporary/platform-tools/platform-tools/libwinpthread-1.dll" "$output_root/release/tools/"
    cp "$temporary/platform-tools/platform-tools/NOTICE.txt" "$output_root/release/tools/PLATFORM-TOOLS-NOTICE.txt"
  ...
  linux-x64)
    ...
    cp "$temporary/platform-tools/platform-tools/fastboot" "$output_root/release/tools/fastboot"
    cp "$temporary/platform-tools/platform-tools/NOTICE.txt" "$output_root/release/tools/PLATFORM-TOOLS-NOTICE.txt"
    chmod +x "$output_root/install.sh" "$output_root/bin/jackrabbit-installer" "$output_root/release/tools/fastboot"
    ;;
  macos-x64|macos-arm64)
    ...
    cp "$temporary/platform-tools/platform-tools/fastboot" "$output_root/release/tools/fastboot"
    cp "$temporary/platform-tools/platform-tools/NOTICE.txt" "$output_root/release/tools/PLATFORM-TOOLS-NOTICE.txt"
    chmod +x "$output_root/install.sh" "$output_root/install.command" "$output_root/bin/jackrabbit-installer" "$output_root/release/tools/fastboot"
    ;;
esac

cp -al "$release_root/images" "$output_root/release/images"
