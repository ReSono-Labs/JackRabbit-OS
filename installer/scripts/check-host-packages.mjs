...
const { stockR1Release } = await import(pathToFileURL(join(root, "web/src/release/stock-r1-release.mjs")));
...
for (const required of ["Press Enter to configure R1 USB access", "ENTRY INCORRECT. WOULD YOU LIKE TO CANCEL?", 'exec "$installer_binary" install "$release_root"']) {
...
for (const required of ["Press Enter to install or repair", "ENTRY INCORRECT. WOULD YOU LIKE TO CANCEL?", "jackrabbit-installer.exe", "install $Release"]) {
...
const packageRoot = join(root, "dist/packages");
let stagedPackages = [];
try {
...
const packageFiles = {
  "linux-x64": ["README.md", "INSTALL.md", "TROUBLESHOOTING.md", "install.sh", "bin/jackrabbit-installer", "release/tools/fastboot", "drivers/51-jackrabbit-r1.rules"],
  "macos-x64": ["README.md", "INSTALL.md", "TROUBLESHOOTING.md", "install.sh", "install.command", "bin/jackrabbit-installer", "release/tools/fastboot"],
  "macos-arm64": ["README.md", "INSTALL.md", "TROUBLESHOOTING.md", "install.sh", "install.command", "bin/jackrabbit-installer", "release/tools/fastboot"],
  "windows-x64": ["README.md", "INSTALL.md", "TROUBLESHOOTING.md", "install.cmd", "install.ps1", "install-drivers.ps1", "bin/jackrabbit-installer.exe", "release/tools/fastboot.exe"]
};

for (const packageName of stagedPackages) {
  const match = /^jackrabbit-(linux-x64|macos-x64|macos-arm64|windows-x64)-current-v0\.1$/.exec(packageName);
  if (!match) fail("STAGED-NAME", `unexpected package directory ${packageName}`);
  ...
  const presentImages = (await Promise.all(["stock", "jackrabbit", "cipheros"].map(name => readdir(join(staged, "release/images", name))))).flat();
  if (presentImages.length !== stockR1Release.artifacts.length) fail("STAGED-IMAGES", `${packageName} does not contain exactly ${stockR1Release.artifacts.length} images`);
  for (const artifact of stockR1Release.artifacts) {
    const imagePath = join(staged, "release", artifact.path);
    const info = await stat(imagePath);
    if (info.size !== artifact.size) fail("STAGED-IMAGE-SIZE", `${packageName}/${artifact.path} has the wrong size`);
    if (await sha256(imagePath) !== artifact.sha256) fail("STAGED-IMAGE-HASH", `${packageName}/${artifact.path} has the wrong SHA-256`);
  }
