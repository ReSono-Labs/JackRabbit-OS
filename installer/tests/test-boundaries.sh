...
ln -s /tmp "$symlink_fixture/contracts/escape"
expect_fail "symlink escape" "$symlink_fixture" "SYMLINK"

parent_fixture="$(new_fixture parent-reference)"
printf '%s\n' 'const ESCAPE: &str = "../../../runtime/example";' > "$parent_fixture/cli/src/escape.rs"
...
printf '%s\n' '{"dependencies":{"product":"file:../product"}}' > "$dependency_fixture/cli/package.json"
