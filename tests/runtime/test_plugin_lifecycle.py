...
class PluginLifecycleTest(unittest.TestCase):
    ...
    def test_same_name_replacement_keeps_one_catalog_item(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            first=life.preflight(_inspection(root,"one")); installed=life.confirm(first.token,audience=AgentAudience.VOICE,replace=False,changed_by="owner",reason="install")
            self.assertEqual("enabled", installed.lifecycle_state)
